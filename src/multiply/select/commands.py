...
def main(result_dir, algorithm):
    ...
    algo_costs = [m.cost for m in explorer.uniq_multiplexes] # NB: only looking at unique
    rnd_costs = [m.cost for m in rnd_explorer.uniq_multiplexes]
    print(f"  {'Algorithm':>10}  {'Mean Cost':>10}  {'Lowest Cost':>10}")
    print(f"  {algorithm:>10}  {np.mean(algo_costs):>10.3f}  {np.min(algo_costs):>10.3f}")
    print(f"  {'Random':>10}  {np.mean(rnd_costs):>10.3f}  {np.min(rnd_costs):>10.3f}\n")
    plot_explorer_costs(algorithm, algo_costs, rnd_costs, output_path=f"{output_dir}/search.diagnostics.png")
    ...
    for ix, multiplex in enumerate(explorer.top_multiplexes):
        ...
        multiplex_df.to_csv(f"{multiplex_output_dir}/table.{multiplex_name}_overview.csv", index=False)
        
        # Get pairwise dataframe
        pairwise_df = pairwise_costs[0].primer_values.loc[primer_names, primer_names]
        visualise_pairwise_costs(pairwise_df,
                                cbar_title="Interaction Score",
                                output_path=f"{multiplex_output_dir}/plot.{multiplex_name}.primer_interactions.pdf"
                                )
        multiplex_align_df = (alignments_df
                            .query("primer1_name in @primer_names and primer2_name in @primer_names"))
